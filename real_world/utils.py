...
def pick_place_primitive_helper(arm, pick_pose, place_pose,
                                backup=0.05, **kwargs):

    arm.open_grippers(blocking=True)
    pick = deepcopy(pick_pose)
    if not arm.movej(
            params=pick, blocking=True,
            use_pos=True, **kwargs):
        return False
    arm.close_grippers(blocking=True)
    ...
    if not arm.movel(params=post_grasp_pose, blocking=True, use_pos=True,
                     **post_grasp_kwargs):
        return False
    if not arm.movej(
